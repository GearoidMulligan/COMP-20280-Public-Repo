...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	...
	public V put(K key, V value) throws IllegalArgumentException {
	    ...
	    Position<Entry<K,V>> entryPos = treeSearch(root(), key);
	    if (isExternal(entryPos)) {                    // key is new
	      expandExternal(entryPos, newEntry);
	      return null;
	    } else if(entryPos.getElement()==null){ 
	    	return null;
	    }
	    else{
	      V old = entryPos.getElement().getValue();
	      set(entryPos, newEntry);
	      rebalanceAccess(entryPos);                   // hook for balanced tree subclasses
	      return old;
	    }
		/*Entry<K,V> newEntry = new MapEntry<>(key, value);
		Position<Entry<K,V>> entryPos = treeSearch(root(), key);
		if (isExternal(entryPos)) {
